...
iPoint j1Map::MapToWorld(int x, int y) const
{
	...
	case MAPTYPE_ISOMETRIC:
		LOG("Map type = isometric");
		ret.x = x * data.tile_width / 2 - y * data.tile_width / 2;
		ret.y = x * data.tile_height / 2 + y * data.tile_height / 2;
		break;
	case MAPTYPE_ORTHOGONAL:
		LOG("Map type = orthogonal");
		ret.x = x * data.tile_width;
		ret.y = y * data.tile_height;
		break;
	case MAPTYPE_STAGGERED:
		LOG("Map type = staggered");
		break;
...
iPoint j1Map::WorldToMap(int x, int y) const
{
	...
	case MAPTYPE_ISOMETRIC:
		LOG("Map type = isometric");
		xy_1 = x / (data.tile_width * 0.5);
		xy_2 = y / (data.tile_height * 0.5);
		ret.x = (xy_1 + xy_2) / 2;
		ret.y = xy_2 - ret.x;
		break;
	case MAPTYPE_ORTHOGONAL:
